...
A library of explanatory video overlay templates that render through Hyperframes. Each template is a self-contained HTML composition with `data-*` timing, CSS layout, and a paused GSAP timeline; it animates in over 5–12 seconds and either fills the frame or sits as a card on top of underlying footage.
...
2. **Still-frame discipline.** Every mockup is the most information-dense moment of the overlay. Motion is added later in Hyperframes.
